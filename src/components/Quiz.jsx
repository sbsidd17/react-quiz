...
function Quiz({ categoryId, questionCount, categoryName }) {
  // State variables
  ...
  // API URL
  let url = `https://opentdb.com/api.php?amount=${questionCount}&category=${categoryId}&type=multiple`;
    ...
    <>
      <div className="flex flex-col md:flex-row">
        ...
        <div className="h-[calc(100vh-4rem)] md:w-[70%] flex flex-col justify-between">
          <div>
            {/* Category Section */}
            <QuizSection categoryName={categoryName} />
            {/* Quiz Header */}
            <div>
            ...
            <div className="m-5 overflow-y-auto">
                ...
                {Quizdata.length > 0 && (
                  <span
                    dangerouslySetInnerHTML={{
                      __html: Quizdata[questionIndex]?.question,
                    }}
                  ></span>
                )}
              </div>
              <div className="m-3">
                {Quizdata.length > 0 &&
                  Quizdata[questionIndex].incorrect_answers?.map(
                    (item, index) => {
                      return (
                        <label key={index}>
                          <input
